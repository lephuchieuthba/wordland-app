...
* **Phủ định / Nghi vấn:** `S + do/does + not + V_bare` | `Do/Does + S + V_bare?`
...
* Diễn tả hành động sắp xảy ra trong tương lai gần (có kế hoạch từ trước).
* Diễn tả sự phàn nàn về thói quen xấu (đi với *always*).

**3. Dấu hiệu nhận biết:**
* *Now, right now, at the moment, at present, Listen!, Look!, Keep silent!...*
""",
...
**2. Cách dùng:**
* Diễn tả hành động đã xảy ra trong quá khứ nhưng kết quả còn lưu ở hiện tại.
* Diễn tả trải nghiệm, kinh nghiệm tính đến thời điểm hiện tại.
* Diễn tả hành động bắt đầu ở quá khứ và vẫn còn tiếp diễn ở hiện tại.

**3. Dấu hiệu nhận biết:**
* *Already, yet, just, ever, never, since + mốc thời gian, for + khoảng thời gian, recently, so far...*
""",
...
**2. Cách dùng:**
* Nhấn mạnh tính **liên tục**, kéo dài của hành động bắt đầu từ quá khứ và vẫn tiếp diễn ở hiện tại.

**3. Dấu hiệu nhận biết:**
* *All day/week, since, for, how long...*
""",
...
* **Phủ định / Nghi vấn:** `S + did + not + V_bare` | `Did + S + V_bare?`
...
* Diễn tả một hành động đang xảy ra thì một hành động khác chen vào (dùng *When/While*).

**3. Dấu hiệu nhận biết:**
* *At + giờ + thời gian quá khứ (At 8 PM yesterday), at this time last week, while, when...*
""",
...
**2. Cách dùng:**
* Diễn tả một hành động xảy ra và hoàn thành **trước** một hành động khác hoặc một mốc thời gian trong quá khứ.

**3. Dấu hiệu nhận biết:**
* *Before, after, by the time, as soon as, by the end of...*
""",

    "Past Perfect Continuous (QKHT tiếp diễn)": """
...
**2. Cách dùng:**
* Nhấn mạnh khoảng thời gian và tính tiếp diễn của một hành động xảy ra trước một hành động khác trong quá khứ.

**3. Dấu hiệu nhận biết:**
* *Until then, by the time, prior to that time, for + khoảng thời gian...*
""",
...
**1. Công thức:**
* **Khẳng định:** `S + will + V_bare + O`
* **Phủ định:** `S + will + not (won't) + V_bare`
* **Nghi vấn:** `Will + S + V_bare?`

**2. Cách dùng:**
* Diễn tả quyết định bộc phát ngay tại thời điểm nói.
* Diễn tả dự đoán không có căn cứ cụ thể.
* Diễn tả lời hứa, lời mời hoặc đe dọa.

**3. Dấu hiệu nhận biết:**
* *Tomorrow, next week/month, in the future, think, believe, promise...*
""",
...
**3. Dấu hiệu nhận biết:**
* *At this time tomorrow, at + giờ + thời gian tương lai (At 9 AM next Sunday)...*
""",
...
**3. Dấu hiệu nhận biết:**
* *By the time, by + mốc thời gian tương lai (By next year), by the end of this month...*
""",

    "Future Perfect Continuous (TLHT tiếp diễn)": """
...
**2. Cách dùng:**
* Diễn tả một hành động kéo dài liên tục đến một mốc thời gian nhất định trong tương lai.

**3. Dấu hiệu nhận biết:**
* *By... for + khoảng thời gian (By next month, I will have been working here for 5 years).*
"""
}
...
def generate_ai_content_cached(prompt: str):
    try:
        ...
        return ""
# --- HÀM TẠO FLASHCARDS CÓ CACHE CHUẨN (KHÔNG LƯU KHI LỖI/RỖNG) ---
@st.cache_data(ttl=86400, show_spinner=False)
def get_cached_flashcards_fixed(topic: str, level: str):
    prompt = f"Tạo 5 từ vựng tiếng Anh chủ đề '{topic}' phù hợp trình độ '{level}'. Trả về JSON: [{{\"icon\": \"🍎\", \"word\": \"Apple\", \"ipa\": \"/ˈæp.əl/\", \"meaning\": \"Quả táo\", \"example\": \"I eat an apple.\"}}]"
    
    try:
        # Gọi Gemini trực tiếp trong hàm cache này
        config = types.GenerateContentConfig(response_mime_type="application/json")
        res = client.models.generate_content(
            model=MODEL_NAME,
            contents=prompt,
        ...
        raw_text = res.text if res and hasattr(res, 'text') else ""
    ...
    # Trả về None để tránh Streamlit lưu dữ liệu rỗng vào Cache
    return None
# --- CUSTOM CSS ---
st.markdown("""
<style>
...
# --- HÀM XỬ LÝ DỮ LIỆU TIN NHẮN CHAT (CHATS JSON) ---
...
def send_group_message(group_name, sender_user, sender_name, message_text):
...
def admin_send_ai_homework(admin_user, target_student, skill_type, topic, level):
    try:
        ...
        cleaned = clean_json_text(raw_text)
        if not cleaned:
            return False, "⚠️ Không nhận được dữ liệu từ Gemini API. Vui lòng thử lại!"
...
if st.session_state.is_admin:
    ...
    with tab_adm_grading:
        ...
        if submitted_hw:
            ...
            # --- HIỂN THỊ ĐỀ BÀI DẠNG WEB THÂN THIỆN MÀU NỔI BẬT ---
...
# DANH SÁCH 15 TAB HỌC TẬP (ĐÃ XÓA TAB XEM TẤT CẢ CÁC NHÓM)
tab_cards, tab_speaking, tab_grammar_learn, tab_tense_game, tab_grammar_check, tab_reading, tab_dialogue, tab_writing, tab_dict, tab_fav, tab_mock_exam, tab_maze_game, tab_analytics, tab_groups, tab_user_mailbox = st.tabs([
    "🃏 Flashcards", "🎙️ Luyện Phát Âm", "📚 Học Ngữ Pháp AI", "⚙️ Đấu Trường Ngữ Pháp", 
    "📝 Sửa Lỗi Ngữ Pháp", "📖 Đọc Hiểu", "🎧 Hội Thoại", "✍️ Thử Thách Viết Văn", 
    "📚 Từ Điển & Dịch", "❤️ Từ Vựng Yêu Thích", "📝 Đề Thi Thử 4 Kỹ Năng", "🎮 Mê Cung Quái Vật", "📊 Thống Kê & Tiến Độ", "👥 Nhóm Học Tập", "📬 Hộp Thư Nhận Lời Mời & Bài Tập"
])

# 1. FLASHCARDS (ĐÃ THAY CƠ CHẾ CACHE CHỐNG LỖI RỖNG)
with tab_cards:
    st.subheader(f"📌 Thẻ từ vựng sinh động ({st.session_state.level}) - Chủ đề: {selected_topic}")
    
    col_fc1, col_fc2 = st.columns([2, 1])
    with col_fc1:
        btn_create = st.button("✨ Lấy / Tạo bộ Flashcards", type="primary", key="btn_fc")
    with col_fc2:
        # Nút xóa Cache nếu muốn ép AI tạo từ mới
        if st.button("🔄 Xóa Cache từ vựng chủ đề này", key="btn_clear_fc_cache"):
            get_cached_flashcards_fixed.clear()
            st.success("Đã xóa Cache! Bấm nút bên trái để tạo lại.")

    if btn_create:
        with st.spinner("Đang tải dữ liệu từ vựng..."):
            cards_data = get_cached_flashcards_fixed(selected_topic.strip(), st.session_state.level.strip())
            
            if cards_data:
                st.session_state.flashcards = cards_data
                trigger_confetti()
            else:
                # Nếu API rỗng, xóa cache lần thất bại này để lần sau bấm lại sẽ gọi lại API
                get_cached_flashcards_fixed.clear()
                st.error("⚠️ Không thể kết nối Gemini API hoặc quá hạn mức Quota. Vui lòng thử lại sau ít phút!")

    # Hiển thị bộ Flashcards
...
# 3. HỌC NGỮ PHÁP (SỬ DỤNG DỮ LIỆU CỐ ĐỊNH, KHÔNG GỌI API)
with tab_grammar_learn:
    st.subheader(f"📚 Học Lý Thuyết Ngữ Pháp: **{selected_tense}**")
    
    if st.button("📖 Xem Bài Giảng Ngữ Pháp", type="primary", key="btn_learn_grammar"):
        # Lấy bài giảng từ từ điển dữ liệu của bạn
        lesson_content = MY_GRAMMAR_DATA.get(
            selected_tense, 
            "⚠️ Nội dung lý thuyết cho thì này đang được cập nhật. Vui lòng chọn thì khác!"
        )
        st.session_state.grammar_lesson = lesson_content
        add_xp(5)
        st.rerun()
        ...
        with st.spinner("AI Gemini đang biên soạn bài đọc hiểu..."):
            ...
            try:
                ...
                if cleaned:
                    ...
                    st.error("⚠️ Hệ thống tạm thời không nhận được dữ liệu từ AI. Hãy bấm nút 'Tạo Bài Đọc Mới' để thử lại!")
            except json.JSONDecodeError:
                st.error("⚠️ Phản hồi chưa chuẩn định dạng. Vui lòng nhấn nút thử lại!")
            except Exception as e:
                st.error(f"Lỗi: {e}")
...
# 12. TRÒ CHƠI MÊ CUNG QUÁI VẬT (10x10 & ĐIỀU KHIỂN NÚT NHẤN)
with tab_maze_game:
    st.subheader("🎮 Trò Chơi: Mê Cung Quái Vật 10x10")
    st.caption("💡 *Di chuyển nhân vật 🧙‍♂️ tìm lối thoát 🚪. Đụng độ quái vật 👾 để trả lời câu hỏi Tiếng Anh và nhận điểm XP!*")

    MAZE_SIZE = 10

    def generate_custom_maze(size=10):
        ...
        for r in range(2, size - 2, 2):
            for c in range(2, size - 2, 2):
                maze[r][c] = 1
                if random.random() > 0.4 and r + 1 < size - 1:
                    maze[r + 1][c] = 1
                elif c + 1 < size - 1:
                    maze[r][c + 1] = 1
        random.seed()
    ...
    if "maze_grid" not in st.session_state or len(st.session_state.get("maze_grid", [])) != MAZE_SIZE:
        st.session_state.maze_grid = generate_custom_maze(MAZE_SIZE)
        st.session_state.player_pos = [1, 1]
        st.session_state.exit_pos = [MAZE_SIZE - 2, MAZE_SIZE - 2]

        num_monsters = random.randint(9, 15)
        available_cells = []
        for r in range(1, MAZE_SIZE - 1):
            for c in range(1, MAZE_SIZE - 1):
                if st.session_state.maze_grid[r][c] == 0 and [r, c] != [1, 1] and [r, c] != [MAZE_SIZE - 2, MAZE_SIZE - 2]:
                    available_cells.append([r, c])
        
        actual_monsters_count = min(num_monsters, len(available_cells))
        st.session_state.monsters_pos = random.sample(available_cells, actual_monsters_count)
        st.session_state.current_battle_monster = None

    def move_player(dr, dc):
        if st.session_state.get("current_battle_monster"):
            st.warning("⚠️ Bạn đang đụng độ quái vật! Hãy giải đố để vượt qua trước khi tiếp tục di chuyển.")
            return

        r, c = st.session_state.player_pos
        nr, nc = r + dr, c + dc
        
        if 0 <= nr < MAZE_SIZE and 0 <= nc < MAZE_SIZE and st.session_state.maze_grid[nr][nc] == 0:
            st.session_state.player_pos = [nr, nc]
            if [nr, nc] in st.session_state.monsters_pos:
                st.session_state.current_battle_monster = [nr, nc]

    st.markdown(f"#### 👾 Quái vật còn lại trong mê cung: **{len(st.session_state.monsters_pos)}** con")
    ...
    for r in range(MAZE_SIZE):
        ...
        for c in range(MAZE_SIZE):
            if [r, c] == st.session_state.player_pos:
                row_str += "🧙‍♂️"
            elif [r, c] == st.session_state.exit_pos:
                row_str += "🚪"
            elif [r, c] in st.session_state.monsters_pos:
                row_str += "👾"
            elif st.session_state.maze_grid[r][c] == 1:
                row_str += "🧱"
            else:
                row_str += "🟩"
        maze_display.append(row_str)

    st.markdown(
        f"""
        <div style="background-color: #1e272e; padding: 20px; border-radius: 15px; font-family: monospace; font-size: 24px; line-height: 1.2; letter-spacing: 4px; text-align: center; overflow-x: auto; white-space: nowrap; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
            {"<br>".join(maze_display)}
        </div>
        """, 
        unsafe_allow_html=True
    )
    ...
    with col_c2:
        st.markdown("<h5 style='text-align: center;'>🕹️ <b>Bảng Điều Khiển Di Chuyển</b></h5>", unsafe_allow_html=True)
        
        c_u1, c_u2, c_u3 = st.columns([1, 1, 1])
        with c_u2:
            if st.button("⬆️ Lên", use_container_width=True, key="btn_mv_up"):
                move_player(-1, 0)
                st.rerun()

        c_m1, c_m2, c_m3 = st.columns([1, 1, 1])
        with c_m1:
            if st.button("⬅️ Trái", use_container_width=True, key="btn_mv_left"):
                move_player(0, -1)
                st.rerun()
        with c_m2:
            if st.button("⬇️ Xuống", use_container_width=True, key="btn_mv_down"):
                move_player(1, 0)
                st.rerun()
        with c_m3:
            if st.button("➡️ Phải", use_container_width=True, key="btn_mv_right"):
                move_player(0, 1)
                st.rerun()

        st.write("")
        if st.button("🔄 Tạo Lại Mê Cung Mới", use_container_width=True, key="btn_reset_full_maze"):
            del st.session_state.maze_grid
            st.rerun()

    if st.session_state.player_pos == st.session_state.exit_pos:
        trigger_confetti()
        st.balloons()
        st.success("🎉 CHÚC MỪNG BẠN ĐÃ THOÁT KHỎI MÊ CUNG THÀNH CÔNG! (+100 XP)")
        add_xp(100)

    if st.session_state.current_battle_monster:
        st.divider()
        st.markdown("### ⚔️ **ĐỤNG ĐỘ QUÁI VẬT!**")
        
        if "maze_battle_quiz" not in st.session_state:
            with st.spinner("Quái vật 👾 đang ra câu hỏi đố vui tiếng Anh..."):
                prompt_battle = f"Tạo 1 câu hỏi trắc nghiệm tiếng Anh cho trình độ '{st.session_state.level}'. Trả về JSON: {{\"question\": \"...\", \"options\": [\"A...\", \"B...\", \"C...\", \"D...\"], \"answer\": \"A\"}}"
                try:
                    raw_text = generate_ai_content_cached(prompt_battle)
                    cleaned = clean_json_text(raw_text)
                    if cleaned:
                        st.session_state.maze_battle_quiz = json.loads(cleaned)
                    else:
                        st.error("⚠️ API AI rỗng!")
                except Exception as e:
                    st.error(f"Lỗi trận đấu: {e}")

        if "maze_battle_quiz" in st.session_state and st.session_state.maze_battle_quiz:
            bq = st.session_state.maze_battle_quiz
            st.markdown(f"""
            <div class="reading-box" style="border-left-color: #d63031;">
                <h4 style="color: #d63031;">👾 Quái Vật Ngáng Đường!</h4>
                <p style="font-size: 1.1rem; font-weight: bold;">"{bq.get('question')}"</p>
            </div>
            """, unsafe_allow_html=True)

            user_b_ans = st.radio("Chọn đáp án chính xác để tiêu diệt quái vật:", bq.get("options", []), key="radio_battle_ans")
            
            if st.button("💥 TẤN CÔNG BẰNG ĐÁP ÁN", type="primary", key="btn_submit_battle"):
                if user_b_ans and user_b_ans[0] == bq.get("answer", "").strip().upper():
                    trigger_confetti()
                    st.success("🎉 Chính xác! Bạn đã tiêu diệt Quái vật 👾 và nhận được +20 XP!")
                    add_xp(20)
                    
                    st.session_state.monsters_pos.remove(st.session_state.current_battle_monster)
                    st.session_state.current_battle_monster = None
                    del st.session_state.maze_battle_quiz
                    st.rerun()
                else:
                    st.error("❌ Đáp án chưa đúng! Quái vật đã chống đỡ được, hãy thử chọn đáp án khác.")
...
# 14. TẠO NHÓM, THÀNH VIÊN & CHAT NHÓM THỜI GIAN THỰC
with tab_groups:
    st.subheader("👥 Nhóm Học Tập, Bảng Xếp Hạng & Khung Chat Nhóm")
    ...
    sub_g1, sub_g2 = st.tabs(["🚀 Khung Nhóm & Chat Nhóm", "➕ Xin Vào Nhóm / Tạo Nhóm Mới"])
    ...
    with sub_g1:
        if my_group_name:
            ...
            col_left_group, col_right_chat = st.columns([1.1, 1.2])
            ...
            with col_left_group:
                ...
                if non_member_students:
                    target_friend = st.selectbox("Chọn học sinh muốn mời:", non_member_students, key="sel_friend_invite")
                    if st.button("📩 Gửi Lời Mời Nhóm", type="primary", use_container_width=True, key="btn_send_friend_inv"):
                        ok, msg = send_group_invite(sender=user_curr, recipient=target_friend, group_name=my_group_name, invite_type="invite_user")
                        if ok:
                            trigger_confetti()
                            st.success(f"🎉 Đã gửi lời mời tham gia nhóm tới `{target_friend}`!")
                        else:
                            st.warning(msg)
                else:
                    st.info("Tất cả học sinh đã có trong nhóm!")

                if is_leader:
                    st.divider()
                    st.markdown("##### 🛠️ **Quản Lý Thành Viên (Trưởng Nhóm):**")
                    removable_members = [m for m in members_list if m != user_curr]
                    if removable_members:
                        user_to_remove = st.selectbox("Chọn thành viên cần xóa:", removable_members, key="sel_rem_mem")
                        if st.button("❌ Mời Ra Khỏi Nhóm", type="primary", key="btn_rem_mem"):
                            ...
                            save_groups_data(groups)
                            st.success(f"Đã xóa `{user_to_remove}` khỏi nhóm!")
                            st.rerun()
                ...
                if st.button("🚪 Rời Khỏi Nhóm Này", key="btn_leave_group"):
                    if is_leader:
                        ...
                        st.warning("Trưởng nhóm rời đi! Nhóm đã giải tán.")
                    else:
            ...
            with col_right_chat:
                st.markdown(f"##### 💬 **Khung Chat Nhóm: {my_group_name}**")
                
                c_head1, c_head2 = st.columns([3, 1])
                with c_head2:
                    if st.button("🔄 Tải lại tin", key="btn_refresh_chat"):
                        st.rerun()

                chats_all = load_chats_data()
                group_chats = chats_all.get(my_group_name, [])

                chat_html = ""
                if group_chats:
                    for msg in group_chats[-15:]:
                        is_me = (msg.get("sender_user") == user_curr)
                        align_style = "margin-left: auto; background-color: #6C5CE7; color: white;" if is_me else "margin-right: auto; background-color: #f1f2f6; color: #2d3436;"
                        sender_label = "Bạn" if is_me else f"{msg.get('sender_name')} (@{msg.get('sender_user')})"
                        
                        chat_html += f"""
                        <div style="max-width: 80%; padding: 10px 14px; border-radius: 15px; margin-bottom: 8px; {align_style} font-size: 0.95rem; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                            <div style="font-size: 0.75rem; font-weight: bold; opacity: 0.8; margin-bottom: 3px;">{sender_label} • <span style="font-weight: normal;">{msg.get('time')}</span></div>
                            <div>{msg.get('message')}</div>
                        </div>
                        """
                else:
                    chat_html = "<p style='text-align: center; color: #a4b0be; padding: 20px;'>💬 Chưa có tin nhắn nào. Hãy mở lời chào cả nhóm nhé!</p>"

                st.markdown(f"""
                <div style="background-color: #ffffff; padding: 15px; border-radius: 15px; height: 350px; overflow-y: auto; border: 2px solid #e1b12c; margin-bottom: 12px; display: flex; flex-direction: column;">
                    {chat_html}
                </div>
                """, unsafe_allow_html=True)

                with st.form(key=f"form_chat_{my_group_name}", clear_on_submit=True):
                    col_input, col_btn = st.columns([3.5, 1])
                    with col_input:
                        chat_msg_input = st.text_input("Nhập tin nhắn...", placeholder="Gửi lời nhắn cho nhóm...", label_visibility="collapsed")
                    with col_btn:
                        submit_chat = st.form_submit_button("📤 Gửi", use_container_width=True)
                        
                    if submit_chat and chat_msg_input.strip():
                        send_group_message(
                            group_name=my_group_name,
                            sender_user=user_curr,
                            sender_name=st.session_state.fullname,
                            message_text=chat_msg_input.strip()
                        )
                        st.rerun()
            ...
            with c_g1:
                ...
                if st.button("🚀 Khởi Tạo Nhóm", type="primary", key="btn_create_g"):
                    if not create_g_name.strip() or not create_g_pass.strip():
                        ...
                        send_group_message(create_g_name.strip(), "system", "Hệ thống Wordland", f"🎉 Nhóm {create_g_name.strip()} đã được khởi tạo thành công!")
                ...
                if st.button("📩 Gửi Yêu Cầu Vào Nhóm", key="btn_join_g"):
                    if join_g_name == "-- Chọn nhóm --" or not join_g_pass.strip():
                        ...
                        if join_g_pass.strip() == target_g.get("passcode"):
                            ...
                            if ok:
                                trigger_confetti()
                                st.success(f"🎉 Mã chính xác! Yêu cầu tham gia nhóm **{join_g_name}** đã gửi tới `{group_leader}`!")
                            else:
                                st.warning(msg)
                        else:
                            st.error("Mã mật khẩu nhóm không chính xác!")

# 15. HỘP THƯ HỌC SINH (BAO GỒM CẢ XEM ĐIỂM VÀ LÀM BÀI TẬP AI GIAO)
